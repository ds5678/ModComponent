...
namespace ModComponent.Utils
{
	internal static class ConversionUtils
	{
		internal static Standard.List<T> Convert<T>(Il2Cpp.List<T> list)
		{
			...
			foreach (var element in list) result.Add(element);
			return result;
		}
		...
		internal static Il2Cpp.List<T> Convert<T>(T[] array)
		{
			...
			foreach (var element in array) result.Add(element);
			return result;
		}
		internal static Standard.List<T> Convert<T>(Il2Cpp.IEnumerable<T> enumerable)
		{
			...
			foreach (var element in temp) result.Add(element);
			return result;
		}
		...
		internal static T[] ToArray<T>(Il2Cpp.IEnumerable<T> enumerable) => ToArray<T>(new Il2Cpp.List<T>(enumerable));
	}
}
